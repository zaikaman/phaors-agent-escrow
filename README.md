...
Use these prompts in three fresh agent sessions to test the real reusable skill path on Pharos Atlantic. The prompts are intentionally short: the skill should supply the configured testnet agents, network defaults, escrow address, USDC address, metadata validation, `doctor` preflights, signer selection, proof hashing, and release policy.
...
The task is to produce a concise markdown brief explaining how pharos-agent-escrow lets AI agents coordinate paid work on-chain.

Do not run demo scripts. Use the skill's production workflow and return the work order ID plus the next prompt for the Worker Agent.
```
...
Accept it if it is claimable by the configured worker, write the requested markdown brief, submit proof on-chain, and return the proof URI plus the next prompt for the Verifier Agent.

Do not run demo scripts. Use the skill's production workflow.
```
...
Do not run demo scripts. Use the skill's production verifier workflow and return the decision, release transaction, final status, and worker reputation change.
```

Expected behavior for all three prompts: agents should avoid `judge-demo-usdc`, `judge-demo`, `marketplace-demo`, and `demo`; run role-specific commands; run `doctor` before live writes; validate metadata/proof schemas; use content-addressed `sha256:` artifacts; and return explorer links for on-chain transactions.
